...
\begin{abstract}
Neural guidance for Ant Colony Optimization (ACO) is usually trained as a static prediction, such as a heatmap, but it is deployed inside an iterative search process whose pheromones and incumbent solutions evolve over time. This mismatch limits search adaptivity and weakens zero-shot generalization, since a fixed neural prior learned from one training distribution may become unreliable on benchmark instances with different scales and structures. We first present \DyNACO{}, a dynamic neural ACO model that periodically observes the pheromone distribution and incumbent solution, and updates guidance over candidate edges throughout the search trajectory. Building on \DyNACO{}, we introduce \DyNACS{} (Dynamic Neural Ant Colony Search), which addresses the limitation of using one learned behavior by adding trajectory-diverse guidance. \DyNACS{} represents guidance as several trajectory distributions for different behaviors, allowing different ant groups to express complementary search behaviors within one shared pheromone process. To make the framework tractable at scale, we pair the policy with an ACO backend based on perturbations and a scope-restricted refinement mechanism that jointly ensure efficiency, local convergence, and stable credit assignment. Under a protocol focused on generalization, a single model trained only on 1K-node instances achieves $1.98\%$ average gap on \TSP{} and $2.80\%$ on \CVRP{} across benchmark datasets, outperforming all state-of-the-art neural baselines under matched evaluation. On \TSP{}, \DyNACS{} scales to 100,000-node instances and often reduces total runtime compared with the unguided solver; on \CVRP{}, it consistently improves the unguided baseline with less than $1\%$ neural overhead. These results show that neural guidance can generalize beyond synthetic training distributions when it adapts to the search process and preserves useful diversity in the ant population.
\end{abstract}


\begin{IEEEImpStatement}
Large routing problems arise in logistics, transportation, and network design, where practical instances often differ substantially from the synthetic distributions used to train neural solvers. This work shows that neural guidance can be integrated into an established ant colony optimizer without replacing its search machinery. By adapting guidance during search and maintaining complementary search behaviors, the proposed framework improves robustness under distribution shift while preserving the scalability of the underlying solver. More broadly, the work supports a practical direction for learning-guided optimization: neural components should interact with the internal state of classical solvers, rather than provide one-shot predictions detached from the search process.
...
% ======================================================================
% 3. Dynamic Neural Ant Colony Optimization
% ======================================================================
\section{Dynamic Neural Ant Colony Optimization}
\label{sec:method}
...
This paper presented \DyNACS{}, a dynamic neural ant colony search framework that builds guidance with multiple trajectories on top of \DyNACO{}. The main result is that neural guidance can remain useful throughout an iterative ACO trajectory when it observes the evolving search state and supports multiple complementary search behaviors. Across \TSP{} and \CVRP{} benchmarks, this design improves over the unguided backend and prior neural baselines while preserving the scalability needed for large routing instances.
