...
We evaluate \DyNACS{} with emphasis on zero-shot generalization within each problem type. Unless otherwise specified, a single model trained on uniformly distributed 1K-node synthetic instances is directly tested on benchmark \TSP{} and \CVRP{} instances with different scales, structures, and distributions. This setting differs from the conventional neural combinatorial optimization protocol, where models are often trained and tested on matched synthetic distributions, or where different models are trained for different scales. Our goal is to evaluate whether the learned guidance transfers beyond the training distribution while preserving the efficiency and reliability of the underlying ACO search.

The evaluation follows the progression of the method. We first compare the full \DyNACS{} framework with classical solvers, neural routing solvers, and neural-guided ACO baselines under the zero-shot benchmark protocol. We then isolate the contribution of trajectory-diverse guidance by comparing the shared backend, \DyNACO{} with one learned behavior, and the full \DyNACS{} model. Next, we study the main algorithmic mechanisms through ablations, with particular attention to whether SRR preserves policy credit assignment while maintaining the quality of local search. Finally, we analyze runtime, neural overhead, training efficiency, and learned guidance behavior to clarify how the method scales in practice.
...
This benchmark construction is substantially more challenging than testing on synthetic instances from the training distribution. The training distribution contains only uniform 1K-node instances, whereas the test suite contains benchmark instances with diverse structures and scales. Therefore, strong performance on this suite indicates zero-shot transfer across both scale and distribution.
...
We also include the unguided ACO backend as a direct ablation baseline. This isolates the contribution of neural guidance, since \DyNACO{}, \DyNACS{}, and the unguided backend share the same candidate graph, perturbation mechanism, refinement procedure, and search budget.
...
\smallskip\noindent\textbf{TSP.}
On \TSP{}, \DyNACS{} achieves the best aggregate gap among neural methods while solving all 228 instances. The advantage is most relevant in the medium and large groups, where several end-to-end neural solvers either lose many instances or degrade sharply. The comparison with the ACO backend shows a smaller but consistent gain, indicating that the learned guidance improves the same search environment rather than changing the evaluation budget.

\smallskip\noindent\textbf{CVRP.}
On \CVRP{}, \DyNACS{} also gives the best aggregate neural result and solves all 110 instances. The gain over the ACO backend is larger than on \TSP{} in the total column, suggesting that multiple learned behaviors are particularly useful when capacity constraints create more heterogeneous local decisions. Unlike several neural baselines, the method does not trade quality for the number of solved instances.
...
Classical solvers such as LKH-3, HGS, and AILS-II remain stronger in absolute solution quality, reflecting decades of specialized engineering. The relevant comparison here is therefore not replacement, but whether learning improves a scalable ACO backend under zero-shot evaluation. Table~\ref{tab:exp_proposed} shows that it does, while keeping reliability at 100\% on both problem families.

\subsection{Effect of Trajectory-Diverse Guidance}
\label{sec:trajectory-diverse-effect}

Table~\ref{tab:trajectory-diverse-effect} isolates the progression from the shared search environment to the base model and then to the full model. The unguided backend tests the ACO environment without neural guidance. \DyNACO{} adds Dynamic Neural Ant Colony Optimization through one guidance field that observes the search state. \DyNACS{} builds on \DyNACO{} by allowing multiple learned behaviors to steer disjoint ant groups within the same search process. This comparison is the central test of the second method layer: whether trajectory-diverse guidance improves on the one-behavior model, rather than merely benefiting from the same backend or a larger search budget. The consistent reduction from \DyNACO{} to \DyNACS{} shows that multiple behaviors improve the search process itself: heterogeneous behaviors, shared search memory, and online behavior routing together improve zero-shot benchmark generalization over the dynamic base policy.
...
\midrule
ACO backend & 2.31\%  & 3.18\%  \\
\DyNACO{} (single behavior) & 2.16\% & 3.10\%  \\
\DyNACS{} & \textbf{1.98\%}  & \textbf{2.80\%} \\
\bottomrule
\end{tabular*}
\end{table}
...
Table~\ref{tab:neural-aco} compares \DyNACS{} with prior neural-guided ACO methods at their maximum reported scales. Existing neural ACO methods mainly use static heatmaps or static heuristic priors, while \DyNACS{} updates its guidance throughout the search trajectory and diversifies the trajectories followed by different ant groups within one shared colony. The results show that \DyNACS{} achieves better solution quality while substantially reducing runtime. This supports our central claim that the limitation of prior neural ACO is not only model architecture, but also the static integration strategy.
...
Table~\ref{tab:cross-scale} compares an in-scale model with the 1K-trained model on larger synthetic instances. The 1K model remains close to the in-scale model in all columns and is slightly better in two \TSP{}5K settings. This supports the scale-transfer claim without requiring separate models for each tested size.
