#!/usr/bin/env python3
"""
Test ACO with rho=0.05 and initial pheromone=5.0.
"""
...
def test_aco_with_very_low_decay(n_node=30, n_ants=20, capacity=150.0, device="cuda"):
    """Test ACO with very low decay rate."""
    print(f"\n{'='*60}")
    print(f"Testing ACO with very low decay rate (n={n_node})")
    print(f"{'='*60}\n")
    ...
    for n_iters in iteration_counts:
        ...
        # Create ACO solver with very low decay rate
        aco = MFACO_BPP(
            demand=demand,
            capacity=capacity,
            n_ants=n_ants,
            decay=0.05,  # Very low decay rate
            alpha=1.0,
            beta=1.0,
            elitist=True,
            device=device,
    ...
    print(f"\n{'='*60}")
    print("Summary:")
    print("  - rho=0.05, initial_pheromone=5.0")
    print("  - Check if very low decay rate improves ACO performance")
    print(f"{'='*60}\n")

if __name__ == "__main__":
    device = "cuda" if torch.cuda.is_available() else "cpu"
    test_aco_with_very_low_decay(n_node=30, device=device)
